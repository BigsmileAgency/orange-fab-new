...
db::singleton()->insert('INSERT INTO users(company, website, firstname, lastname, phone, email, activity) VALUES (?, ?, ?, ?, ?, ?, ?)', [$company, $website, $firstname, $lastname, $phone, $email, $activity]);

// STEP 2
$id = db::singleton()->get('SELECT id FROM users WHERE email = ?', [$email]);
$id_user = $id[0]['id'];

$describe_company   = $json->step2->describe_company;
...
$startup_sector   = $json->step2->startup_sector;


db::singleton()->insert('INSERT INTO step2(id_user, describe_company, business_model, major_clients, market_problem, business_create, startup_country, startup_city, startup_people, startup_sector) VALUES (?,?,?,?,?,?,?,?,?,?)'
    , [
        $id_user,
        $describe_company,
        $business_model,
...
// $mail->AddBCC('juliette.malherbe@orange.com', 'juliette.malherbe@orange.com');
// $mail->AddBCC('camille.suratteau@orange.com', 'camille.suratteau@orange.com');
$mail->AddBCC('bruno@bigsmile.be', 'bruno@bigsmile.be');


$mail->Subject = '=?UTF-8?B?'.base64_encode('Thank you for subscribing').'?=';
...
$content = str_replace('{%describe_company%}', $describe_company, $content);
...
$content = str_replace('{%startup_sector%}', $startup_sector, $content);
